    ...
    function component(object) {
      ...
      this.initialize = function () {
        ...
        this.createSource().then(function () {
          ...
          if (['seasonvar', 'lostfilmhd', 'kinotochka', 'kinopub', 'kinoprofi', 'kinokrad', 'kinobase', 'filmix'].indexOf(balanser) == -1) {
      ...
      this.createSource = function () {
        return new Promise(function (resolve, reject) {
          network.silent(window.lampac_localhost + 'lite.js?serial=' + (object.movie.name ? 1 : 0), function (str) {
            try {
              var json = str.replace(/\n|\t|\r/g, '').match(/append = \[(.*?)\]/);
              var splt = json[1].split('},');
              splt.forEach(function (item) {
                var match = item.match(/name:\'(.*?)\',url:\'(.*?)\'/);

                if (match) {
                  var name = match[1].toLowerCase();
                  if (name !== 'jackett') sources[name] = match[2];
                }
              });
      ...
      this.find = function () {
        ...
        this.request(source + (source.indexOf('?') >= 0 ? '&' : '?') + query.join('&'));
      };
      ...
      this.getFileUrl = function (file, call) {
        if (file.method == 'play') call(file.url);else {
      ...
      this.display = function (videos) {
        ...
        this.draw(videos, {
          onEnter: function onEnter(item, html) {
            _this2.getFileUrl(item, function (find_url) {
              if (find_url) {
                var playlist = [];

                var first = _this2.toPlayElement(item);

                first.url = find_url;

                if (item.season) {
                  videos.forEach(function (elem) {
                    var cell = _this2.toPlayElement(elem);

                    if (elem == item) cell.url = find_url;else {
                      if (elem.method == 'call') {
                        cell.url = function (call) {
                          _this2.getFileUrl(elem, function (stream) {
                            cell.url = stream;
          ...
          onContextMenu: function onContextMenu(item, html, data, call) {
            _this2.getFileUrl(item, function (stream) {
              call({
                file: stream,
